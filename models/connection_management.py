...
def _parse_wg_interface_output(output: str) -> Optional[WGConnectionDetails]:
    ...
    try:
        ...
        logger.error(f"Error parsing WireGuard interface output: {e}")
        return None
...
def _find_server_in_db(details: WGConnectionDetails, db: DatabaseClient) -> Tuple[Optional[ServerDBRecord], Optional[str]]:
    ...
    try:
        ...
        logger.error(f"Error finding server in database: {e}")
...
def check_wireguard_status(quiet: bool = False) -> WGStatusReport:
    ...
    try:
        if not quiet:
            print("\nChecking WireGuard status (may require sudo privileges)...")
        ...
        wg_output = subprocess.run(['sudo', 'wg', 'show', 'wg0'], capture_output=True, text=True).stdout
        ...
        if e.returncode == 1 and not quiet:
            print(f"\n{RED}Error: Failed to check WireGuard status. Please ensure you have sudo privileges.{RESET}")
...
def monitor_connection():
    ...
    def curses_main(stdscr):
        try:
            ...
            last_update = 0
            update_interval = 0.9  # 900ms in seconds
...
def show_top_servers(config_manager: ConfigManager) -> List[Dict]:
    """Show top 10 servers globally with lowest load"""
    display_header()
    print("\nTop 10 Global Servers")
    print("=" * 30)
    ...
    try:
        ...
        servers = get_best_servers(
            ...
            max_load=100,  # Consider all servers
...
def select_vpn_endpoint(config_manager: ConfigManager):
    ...
    print("2. Show top 10 servers globally")
...
def select_by_country(config_manager: ConfigManager) -> List[ServerDBRecord]:
    ...
    try:
        ...
        servers = get_best_servers(
            country=country,
            max_load=100,  # Consider all servers
            limit=10,      # Show top 10 from country
            db_path=db_path  # Pass the database path
        )
...
def _perform_disconnect_action() -> bool:
    ...
    """
    print(f"\nExecuting: sudo wg-quick down wg0")
    result = subprocess.run(['sudo', 'wg-quick', 'down', 'wg0'], capture_output=True, text=True)
...
def _perform_connect_action(config_path: Path) -> bool:
    ...
    Returns:
        True if successful, False otherwise
    """
    print(f"\nExecuting: sudo wg-quick up {config_path}")
    result = subprocess.run(['sudo', 'wg-quick', 'up', str(config_path)], capture_output=True, text=True)
...
def _perform_restart_action(config_path: Path) -> bool:
    ...
    # First disconnect
    print(f"\nExecuting: sudo wg-quick down wg0")
    result_down = subprocess.run(['sudo', 'wg-quick', 'down', 'wg0'], capture_output=True, text=True)
    ...
    # Then connect again
    print(f"\nExecuting: sudo wg-quick up {config_path}")
    result_up = subprocess.run(['sudo', 'wg-quick', 'up', str(config_path)], capture_output=True, text=True)
    if result_up.stdout:
...
def _write_config_sudo(config_content: str, config_path: Path) -> bool:
    """Write WireGuard config to file with sudo privileges
    ...
    """
    try:
        # Write config using sudo to ensure proper permissions
        ...
        result = subprocess.run(['sudo', 'cp', str(tmp_path), str(config_path)],
                            capture_output=True, text=True)
                        
        # Verify the config was properly written
        verify_result = subprocess.run(['sudo', 'cat', str(config_path)],
                                    capture_output=True, text=True)
