...
def enable_systemd_service(user_mode: bool = True) -> tuple:
    ...
    try:
        if user_mode:
            result = subprocess.run(['systemctl', '--user', 'enable', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
        else:
            result = subprocess.run(['sudo', 'systemctl', 'enable', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
...
def disable_systemd_service(user_mode: bool = True) -> tuple:
    ...
    try:
        if user_mode:
            result = subprocess.run(['systemctl', '--user', 'disable', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
        else:
            result = subprocess.run(['sudo', 'systemctl', 'disable', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
        ...
        logger.error(f"Failed to disable systemd service: {e}")
...
def start_systemd_service(user_mode: bool = True) -> tuple:
    ...
    try:
        if user_mode:
            result = subprocess.run(['systemctl', '--user', 'start', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
        else:
            result = subprocess.run(['sudo', 'systemctl', 'start', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
...
def stop_systemd_service(user_mode: bool = True) -> tuple:
    ...
    try:
        if user_mode:
            result = subprocess.run(['systemctl', '--user', 'stop', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
        else:
            result = subprocess.run(['sudo', 'systemctl', 'stop', SYSTEMD_SERVICE_NAME], 
                                  capture_output=True, text=True)
...
def manage_autostart(config_manager: ConfigManager) -> None:
    ...
    print(CLEAR_SCREEN)
    print("\nVPN Autostart Configuration (systemd)")
    print("=" * 50)
